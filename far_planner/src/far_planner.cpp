...
void FARMaster::Init() {
    ...
    reset_graph_sub_ = nh_->create_subscription<std_msgs::msg::Empty>(
        "/reset_visibility_graph", 5,
        std::bind(&FARMaster::ResetGraphCallBack, this, std::placeholders::_1));
    odom_sub_ = nh_->create_subscription<nav_msgs::msg::Odometry>(
        "/odom_world", 5, std::bind(&FARMaster::OdomCallBack, this, std::placeholders::_1));
    terrain_sub_ = nh_->create_subscription<sensor_msgs::msg::PointCloud2>(
        "/terrain_cloud", 1, std::bind(&FARMaster::TerrainCallBack, this, std::placeholders::_1));
    scan_sub_ = nh_->create_subscription<sensor_msgs::msg::PointCloud2>(
        "/scan_cloud", 5, std::bind(&FARMaster::ScanCallBack, this, std::placeholders::_1));
    waypoint_sub_ = nh_->create_subscription<geometry_msgs::msg::PointStamped>(
        "/goal_point", 1, std::bind(&FARMaster::WaypointCallBack, this, std::placeholders::_1));
    terrain_local_sub_ = nh_->create_subscription<sensor_msgs::msg::PointCloud2>(
        "/terrain_local_cloud", 1,
        std::bind(&FARMaster::TerrainLocalCallBack, this, std::placeholders::_1));
    joy_command_sub_ = nh_->create_subscription<sensor_msgs::msg::Joy>(
        "/joy", 5, std::bind(&FARMaster::JoyCommandCallBack, this, std::placeholders::_1));
    update_command_sub_ = nh_->create_subscription<std_msgs::msg::Bool>(
        "/update_visibility_graph", 5,
        std::bind(&FARMaster::UpdateCommandCallBack, this, std::placeholders::_1));
    goal_pub_ = nh_->create_publisher<geometry_msgs::msg::PointStamped>("/way_point", 5);
    boundary_pub_ =
        nh_->create_publisher<geometry_msgs::msg::PolygonStamped>("/navigation_boundary", 5);
    ...
    read_command_sub_ = nh_->create_subscription<std_msgs::msg::String>(
        "/read_file_dir", 1, std::bind(&FARMaster::ReadFileCommand, this, std::placeholders::_1));
    save_command_sub_ = nh_->create_subscription<std_msgs::msg::String>(
        "/save_file_dir", 1, std::bind(&FARMaster::SaveFileCommand, this, std::placeholders::_1));

    // DEBUG Publisher
    dynamic_obs_pub_ =
        nh_->create_publisher<sensor_msgs::msg::PointCloud2>("/FAR_dynamic_obs_debug", 1);
    surround_free_debug_ =
        nh_->create_publisher<sensor_msgs::msg::PointCloud2>("/FAR_free_debug", 1);
    surround_obs_debug_ = nh_->create_publisher<sensor_msgs::msg::PointCloud2>("/FAR_obs_debug", 1);
    scan_grid_debug_ =
        nh_->create_publisher<sensor_msgs::msg::PointCloud2>("/FAR_scanGrid_debug", 1);
    new_PCL_pub_ = nh_->create_publisher<sensor_msgs::msg::PointCloud2>("/FAR_new_debug", 1);
    terrain_height_pub_ =
        nh_->create_publisher<sensor_msgs::msg::PointCloud2>("/FAR_terrain_height_debug", 1);
    ...
    main_event_ = nh_->create_wall_timer(std::chrono::milliseconds(int(duration_time * 1000)),
                                         std::bind(&FARMaster::MainLoopCallBack, this));
    planning_event_ = nh_->create_wall_timer(std::chrono::milliseconds(int(duration_time * 1000)),
                                             std::bind(&FARMaster::PlanningCallBack, this));
...
void FARMaster::MainLoopCallBack() {
    ...
    contour_detector_.BuildTerrainImgAndExtractContour(odom_node_ptr_, FARUtil::surround_obs_cloud_,
                                                       realworld_contour_);
    contour_graph_.UpdateContourGraph(odom_node_ptr_, realworld_contour_);
    if (is_graph_init_) {
        if (!FARUtil::IsDebug) printf("\033[2K");
        std::cout << "    " << "Local V-Graph Updated. Number of local vertices: "
                  << ContourGraph::contour_graph_.size() << std::endl;
    }
    ...
    if (is_graph_init_) {
        if (!FARUtil::IsDebug) printf("\033[2K");
        std::cout << "    "
                  << "Number of new vertices adding to global V-Graph: " << new_nodes_.size()
                  << std::endl;
    }
    /* Graph Updating */
    ...
    runtimer_.data =
        FARUtil::Timer.end_time("Total V-Graph Update", is_graph_init_) / 1000.f;  // Unit: second
...
void FARMaster::PlanningCallBack() {
    ...
    if (goal_ptr == NULL) {
        ...
        if (graph_planner_.PathToGoal(goal_ptr, global_path, nav_node_ptr_, current_free_goal,
                                      is_planning_fails, is_reach_goal, is_current_free_nav) &&
            nav_node_ptr_ != NULL) {
...
void FARMaster::LocalBoundaryHandler(const std::vector<PointPair>& local_boundary) {
    ...
    FARUtil::SortEdgesClockWise(robot_pos_,
                                sorted_boundary); /* For better rviz visualization purpose only! */
    for (const auto& edge : sorted_boundary) {
        ...
        boundary_poly.polygon.points.push_back(geo_p1),
            boundary_poly.polygon.points.push_back(geo_p2);
        index_z += 0.001f;  // separate polygon lines
    }
    boundary_pub_->publish(boundary_poly);
}

Point3D FARMaster::ProjectNavWaypoint(const NavNodePtr& nav_node_ptr,
                                      const NavNodePtr& last_point_ptr) {
    ...
    const Point3D extend_p =
        this->ExtendViewpointOnObsCloud(nav_node_ptr_, FARUtil::surround_obs_cloud_, free_dist);
    ...
    if (nav_heading_.norm() > FARUtil::kEpsilon &&
        new_heading.norm_dot(nav_heading_) < 0.0f) {  // negative direction reproject
...
Point3D FARMaster::ExtendViewpointOnObsCloud(const NavNodePtr& nav_node_ptr,
                                             const PointCloudPtr& obsCloudIn, float& free_dist) {
    if (nav_node_ptr->free_direct != NodeFreeDirect::CONVEX || obsCloudIn->empty())
        return nav_node_ptr->position;
    FARUtil::CropPCLCloud(obsCloudIn, viewpoint_around_ptr_, nav_node_ptr->position,
                          free_dist + FARUtil::kNearDist);
    float maxR =
        std::min((nav_node_ptr->position - robot_pos_).norm(), free_dist) - FARUtil::kNearDist;
    ...
    if (!is_wall) {
        ...
        if (viewpoint_around_ptr_->empty()) {
            ...
            bool is_occupied =
                int(FARUtil::PointInXCounter(start_p, R, kdtree_viewpoint_obs_cloud_)) > N_Thred;
            waypoint = start_p;
            while (!is_occupied && ray_dist < free_dist) {
                start_p = start_p + direct * FARUtil::kNearDist;
                ray_dist += FARUtil::kNearDist;
                is_occupied = int(FARUtil::PointInXCounter(start_p, R,
                                                           kdtree_viewpoint_obs_cloud_)) > N_Thred;
                if (ray_dist < maxR) {
                    waypoint = start_p;
                }
            }
...
void FARMaster::LoadROSParams() {
    ...
    master_params_.terrain_range =
        std::min(master_params_.terrain_range, master_params_.sensor_range);
    ...
    if (!FARUtil::IsSameFrameID(odom_frame, master_params_.world_frame)) {
        if (FARUtil::IsDebug)
            RCLCPP_WARN_ONCE(nh_->get_logger(),
                             "FARMaster: odom frame does NOT match with world frame!");
        tf2::Transform odom_to_world_tf_stamp;
        try {
            tf_buffer_->canTransform(master_params_.world_frame, odom_frame, tf2::TimePointZero,
                                     std::chrono::seconds(1));
            auto transform_stamped = tf_buffer_->lookupTransform(master_params_.world_frame,
                                                                 odom_frame, tf2::TimePointZero);
...
void FARMaster::PrcocessCloud(const sensor_msgs::msg::PointCloud2::SharedPtr pc,
    ...
    FARUtil::FilterCloud(cloudOut, master_params_.voxel_dim);  // 体素滤波
    // transform cloud frame
    std::string cloud_frame = pc->header.frame_id;
    FARUtil::RemoveNanInfPoints(cloudOut);  // 提出nan或inf点
    // 将点云转换到世界坐标系下
    if (!FARUtil::IsSameFrameID(cloud_frame, master_params_.world_frame)) {
        if (FARUtil::IsDebug)
            RCLCPP_WARN_ONCE(nh_->get_logger(),
                             "FARMaster: cloud frame does NOT match with world frame!");
        try {
            FARUtil::TransformPCLFrame(cloud_frame, master_params_.world_frame, tf_buffer_,
                                       cloudOut);
...
void FARMaster::ScanCallBack(const sensor_msgs::msg::PointCloud2::SharedPtr scan_pc) {
    if (master_params_.is_static_env || !is_odom_init_)
        return;  // 如果环境是静态的，不考虑动态障碍物或没有odom数据
    this->PrcocessCloud(scan_pc, FARUtil::cur_scan_cloud_);  // 将接收到的点云转换到世界坐标系下
    scan_handler_.UpdateRobotPosition(robot_pos_);  // 根据机器人位置更新地图原点和机器人坐标
}

void FARMaster::TerrainLocalCallBack(const sensor_msgs::msg::PointCloud2::SharedPtr pc) {
    if (master_params_.is_static_env) return;     // 静态地图，不考虑回调
    this->PrcocessCloud(pc, local_terrain_ptr_);  // 将点云转换到世界坐标系
    FARUtil::ExtractFreeAndObsCloud(
        local_terrain_ptr_, FARUtil::local_terrain_free_,
        FARUtil::local_terrain_obs_);  // 按照计算的指标将点云分为障碍物和空闲点云
}
...
void FARMaster::TerrainCallBack(const sensor_msgs::msg::PointCloud2::SharedPtr pc) {
    ...
    if (!is_stop_update_) {
        ...
        FARUtil::CropBoxCloud(
            temp_cloud_ptr_, robot_pos_,
        ...
        // extract new points
        FARUtil::ExtractNewObsPointCloud(temp_obs_ptr_, FARUtil::surround_obs_cloud_,
    ...
    if (!master_params_.is_static_env && !is_stop_update_) {
        ...
        if (int(FARUtil::cur_dyobs_cloud_->size()) > FARUtil::kDyObsThred) {
            if (FARUtil::IsDebug)
                RCLCPP_WARN(nh_->get_logger(), "FARMaster: dynamic obstacle detected, size: %ld",
                            FARUtil::cur_dyobs_cloud_->size());
        ...
        FARUtil::StackCloudByTime(FARUtil::cur_dyobs_cloud_, FARUtil::stack_dyobs_cloud_,
                                  FARUtil::kObsDecayTime, nh_);
    }

    // create and update kdtrees
    FARUtil::StackCloudByTime(FARUtil::cur_new_cloud_, FARUtil::stack_new_cloud_,
                              FARUtil::kNewDecayTime, nh_);
    ...
    planner_viz_.VizMapGrids(neighbor_centers, occupancy_centers, map_params_.cell_length,
                             map_params_.cell_height);
...
void FARMaster::WaypointCallBack(const geometry_msgs::msg::PointStamped& route_goal) {
    if (!is_graph_init_) {
        if (FARUtil::IsDebug)
            RCLCPP_WARN(nh_->get_logger(),
                        "FARMaster: wait for v-graph to init before sending any goals");
        return;
    }
    ...
    if (!FARUtil::IsSameFrameID(goal_frame, master_params_.world_frame)) {
        if (FARUtil::IsDebug)
            RCLCPP_WARN_ONCE(nh_->get_logger(),
                             "FARMaster: waypoint published is not on world frame!");
...
std::unordered_map<NavNodePtr, std::pair<int, std::unordered_set<NavNodePtr>>>
    DynamicGraph::out_contour_nodes_map_;
