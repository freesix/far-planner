...
struct Point3D {
    ...
    float operator*(const Point3D& pt) const { return x * pt.x + y * pt.y + z * pt.z; }
    ...
    float norm() const { return std::hypotf(x, std::hypotf(y, z)); };

    float norm_flat() const { return std::hypotf(x, y); };
...
struct point_comp {
    bool operator()(const Point3D& p1, const Point3D& p2) const { return p1 == p2; }
};
